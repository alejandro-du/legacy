...
main (int argc, char *argv[])
{
	...
	if (argc < 2) {
		puts ("Debe teclear el nombre del archivo PAL.");
		return 1;
	}
	...
	while (!kbhit()) {
	for (numar = 0; numar < 11; numar++) {

	if ((archivo = fopen (sp[numar], "rb")) == NULL) {
		restorecrtmode ();
		printf ("No se puede abrir el sprite: %s", sp[numar]);
		return 1;
	}
	...
	putimage (pos, 20, sprite, COPY_PUT);
