...
main (int argc, char *argv[])
{
	...
	BYTE huge *bloque;
	int x, y, fila = 1, columna = 81, color, xmap, ymap,cod;
	long int cont = 0;
	BYTE num, op;
...
/*	for (color = 0; color < 256; color++) {
	...
	/* Asignar 122880 bytes de memoria dinamica */

	bloque = (BYTE huge *) farmalloc(122881);
	if (bloque == NULL) {
		puts ("Error en asignaci¢n de memoria");
		exit (1);
	}
...
/*	for (cont = 0; cont < (long)122880; cont++) {
	...
	if(fread(bloque, 122880, 1, archivo) != 1) {
		printf ("ERROR.");
		exit (1);
	}
	bloque[122880] = '\0';
	...
	for (x = 0; x < XPANT; x++) {
		for (y = 0; y < YPANT; y++) {
			pintarBLK32x32 (x * 32, y * 32, &bloque[(long)num * 1024]);
			num++;
		}
	}
	fclose (archivo);
